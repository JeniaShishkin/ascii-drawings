...
public:
    explicit Square(char brush, color fgColor, color bgColor, Point topLeftCorner, int size) 
          : ObjectBase(brush, fgColor, bgColor) , m_center(topLeftCorner.GetX() + size / 2,topLeftCorner.GetY() + size / 2),
            corners{ 
              topLeftCorner,
              Point(topLeftCorner.GetX() + size, topLeftCorner.GetY()),
              Point(topLeftCorner.GetX() + size, topLeftCorner.GetY() + size),
              Point(topLeftCorner.GetX(), topLeftCorner.GetY() + size)
          }  {}
